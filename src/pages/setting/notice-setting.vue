...
onMounted(() => {
  ...
  axios.get("/api/setting/get-setting-value/?key=notice_switch")
    .then((res) => {
      notice_switch.value = res.data.data.Res.Value === 'true';
      //console.log(slogan_update_time.value);
    });
})
// 组件销毁时，也及时销毁编辑器
...
<template>
  <div class="flex flex-col m-5">
    <h2>公告管理</h2>
    <v-switch v-model="notice_switch" @click="submit_switch"/>
